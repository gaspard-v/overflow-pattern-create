...
def generate(length: int, pattern_length: int, address_ranges: List[Tuple[int, int]], banned_bytes: List[int] = [0x0], reverse: bool = False) -> Set[Tuple]:
    pattern_return = set()
    s = ()
    nb_empty_set = 0
    while len(pattern_return) < length:
        ...
        if len(s) == 0:
            if nb_empty_set > 5000:
                raise Exception("more than 5000 consecutive empty sets have been generated. \
